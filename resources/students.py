...
class StudentsById(MethodView):
    ...
    def patch(self, student_id):
        ...
        if "password" in new_student_data:
            student_data.password = pbkdf2_sha256.hash(new_student_data["password"])
...
class Login(MethodView):
    def post(self):
        ...
        student = StudentsModel.query.filter(StudentsModel.email == student_data["email"]).first()
        if student:
            if pbkdf2_sha256.verify(student_data["password"], student.password):
